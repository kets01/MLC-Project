...
- [ ] CMake presets: `debug` (sanitizers/tests), `release`, `release-submission` (the exact submitted benchmark configuration). **Deferred pending a decision** — the installed CMake is 4.4.0 so `CMakePresets.json` is available, and it would not change the `cmake_minimum_required(3.10)` floor for plain builds, but it adds a second way to configure the project alongside the README's documented `cmake .. && make`.
