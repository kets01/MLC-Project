...
The most important restriction is that *scatter-gather loads*
...
**Vectorize across rows, loop over columns.**
...
matrix.  Pass 1 reads x for the mean, pass 2 reads x again for variance, and
pass 3 reads x a third time to normalize.  The additional gamma/beta reads
are L1-resident (N-element vectors, typically ≤ 8 KiB).

**Vector length agnostic (VLA).**
``CNTW x8`` queries SVL at runtime.  ``WHILELO p1.s, x_rbase, x_m``
generates the predicate for the tail row chunk (lane j is active iff
r\_base + j < m), so the kernel is correct for any SVL, not only the
...
RMSNorm variants.  (Sprint 8 replaces this silent no-op with a dispatcher
plus a loud precondition failure.)
...
``ε_float × SHIFT × inv_std ≈ 5×10⁻⁴``, so a relative tolerance would be
unfairly strict for elements whose normalized value is near zero.

GiB/s results: V0 baseline (Apple M4, SVL = 512 bits)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Roofline:** the single-core streaming ceiling (``LD1W``/``ST1W`` probe) at
each shape's own footprint, i.e. 104.7–117.2 GiB/s for the six cache-resident
shapes below.  This was originally quoted as one 59.45 GiB/s figure, which is
the ceiling at a 256 MiB working set (Sprint 6 §1.2).
...
bytes, 1R+1W convention), against RMSNorm V0's ~21 %.  The factor of ~2
...
**3R+1W** against RMSNorm's **2R+1W**.  Restated in moved-bytes terms (M=1024,
...
**+50 % in useful-bytes throughput** before any other optimization.
...
- **Kernel:** ``src/norm/layer_norm_ssve.S``, VLA, SME1 mandatory subset only,
  three passes (mean / variance / normalize), column-major contiguous loads
...
against the stable two-pass.  All variants were verified on M4 first, including
large-magnitude stress inputs for V4/V5/V6/Welford (added as gap-fill, since a
Welford speed verdict is only meaningful if its accuracy holds).
...
- **V2 (pre-computed 1/N): +9–12 %**, *much larger than RMSNorm's ≈0*, and
  ...
  serial bubbles are a ~3× larger share of runtime than RMSNorm's single
  once-per-block FDIV.  Hoisting them pays here; it did not there.
- **V4 (4-accumulator ILP): +26–28 %** at N=2048, transferring cleanly from
  RMSNorm (which saw +27–32 %).  Accumulating two statistics does not blunt
  the lever.
- **V5 (load pipelining): ≈0 vs V4**, which also transfers: OOO rename already
  covers it once the chains are broken.
- **V6 (4-row-block contiguity, three passes kept):** ties V4 at
  cache-assisted shapes and gives **+71 % vs V0 in the true-DRAM regime**
...
The comparison production libraries face, measured:

- **Speed: Welford loses everywhere**, by −2.5 % to −8.5 % against V0, and
...
.. admonition:: Corrected in Sprint 6 (§1.3) — "~100× less accurate" does not hold
   ...
   This section originally claimed that Welford is *~100× worse on shifted
   data* and that two-pass therefore "dominates on BOTH axes".  Re-measured
   against a float64 reference at M=16, N=512, across more shift magnitudes:
   ...
   **The stated mechanism was also wrong.**  The original text blames
   ...
   (3.4e-7 against two-pass's 3.0e-6 at shift 1e2 and 1e4).  The extra output
   error comes from the running **mean**, because ``mean += delta/count``
   updates at full input magnitude every element, while two-pass forms the mean
   once and then squares already-centred values.  LayerNorm's output is
...
The centred two-pass subtracts the mean *before* squaring, so its variance
arithmetic works on small values; FP32 Welford updates ``mean`` at full input
magnitude every element, accumulating rounding at the scale of the shift.  Both
are far better than the catastrophic naive ``E[x²]−µ²``, which is the row that
makes this comparison meaningful and which Sprint 7a adds.  Note also that
PyTorch's CPU LayerNorm moved *to* Welford precisely to escape naive:
"Welford ≫ naive" and "Welford ≈ two-pass" are both true, and the original text
compared against the stronger baseline without saying so.
...
both measurements, not only the throughput number.

LayerNorm vs RMSNorm on the same harness 
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Best variant against best variant (both V6), same shapes:
...
**RMSNorm is 1.8–2.3× faster, well beyond the proposal's "10–40 %".**
Two attributable causes: (1) structural traffic, since LayerNorm's three passes
move 4 units (3R+1W) per element against RMSNorm's 3 (2R+1W), a 1.33× floor;
and (2) per-byte efficiency, since LayerNorm adds the mean-subtract and β, and
its two serialization points per block (mean, then variance) cost more than
RMSNorm's one.  In moved-bytes terms at M=1024, N=2048 (16 MiB footprint,
ceiling 115.6 GiB/s) LN V6 sustains ~32.6 GiB/s (**28 %** of that ceiling)
against RMS V6's ~56.6 (**49 %**): the remaining LayerNorm-specific gap is the
pass structure itself.  (Originally quoted as 55 % and 95 %, against the DRAM
...
``inv_std`` is known, which is more state than 32 Z registers hold for real N.
...
LayerNorm (against none on RMSNorm), so LayerNorm is where ZA staging has
structural headroom.  Deeper group depth (8–16 blocks) remains a
shape-specialized JIT decision (Sprint 4), and threading toward the chip
ceiling is Sprint 5.
...
- **Kernels:** ``layer_norm_ssve_v1/v2/v4/v5/v6/welford.S``, all verified
  on M4 (81 test cases, 64 885 assertions in ``test_norm``), with stress
